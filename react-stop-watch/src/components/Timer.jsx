...
export const Timer = ({
  ...
  useEffect(() => {
    if (isTimerClicked && (hours > 0 || minutes > 0 || seconds > 0)) {
  ...
  }, [isTimerClicked, hours, seconds, minutes]);
  ...
  const counterPause = `${!isTimerClicked ? "light-brown" : ""}`;
  return (
    <div className="timer">
      {!isActive ? (
        ...
        <button
          className={`timer-counter ${counterPause}`}
          onClick={() => {
            handlePlay();
            setIsActive(true);
          }}
      ...
      {isActive ? (
        <div className="timer-btns">
          ...
          >
            {isTimerClicked ? <TfiControlPause /> : <TfiControlPlay />}
          </button>
          <button className={pausedClass} onClick={handleReset}>
            <VscDebugRestart />
          </button>
        </div>
      ) : (
